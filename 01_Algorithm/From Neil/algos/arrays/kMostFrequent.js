 ...
 * Time:   O(n) linear because the the methods called inside loops are O(1) constant time, .push and .pop
...
function kMostFrequent(nums, k) {
  ...
  for (const numKey in numToFrequency) {
    ...
    if (frequencyToNums.hasOwnProperty(frequency)) {
      // there could be more than one num with the same frequency
      // which is why the val is an arr
...
// Time: O(n) + O(n) + O(n^2) + O(k) -> O(n^2) quadratic due to sort's worst case
// Space: O(n) linear
function kMostFrequentSort(nums, k) {
  ...
  // buit in Map object, would need to convert the keys back to ints, could do this with .map
