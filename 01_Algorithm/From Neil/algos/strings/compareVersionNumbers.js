 ...
 *          -1 if @v1 smaller
 *          0 if equal
 * Time:    O(n + m + max(n, m)) -> O(n) linear
 *          n = @v1 length, m = @v2 length
 *          .split loops over both, then we loop over
 *          the larger of the two arrays from the .split
 * Space:   O()
 */
...
/**
 * Context: Avoiding using .split first on @v1 and @v2 allows for the possibility
 *          of an earlier exit upon difference in version numbers.
 * Time:    O(n) linear
 *          Despite nested loops because nested loop iterations are increasing
 *          the same index the outer loop is using.
 * Space:   O(n)
 *          As v1 or v2 grow is size v1Revision or v2Revision
