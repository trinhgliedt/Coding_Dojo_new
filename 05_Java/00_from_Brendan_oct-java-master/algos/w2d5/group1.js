...
// Plan:
// Steps:
// 1. We have an arry, arr. Plan is to sort this in ascending order using arr.sort().
// 2. the minimum is always the first element arr[0]
// push - put it into the array  arr.push(x),  arr.sort()
// pop  - we look for the element in the array and do a splice to remove it.
//
// minStack() - we just return arr[0]
...
class minStack {
    ...
    push(node) {
        //every time we add, we also add this to the array then sort it ascending
    ...
    pop() {
        ...
        // we need to find the element in the array and splice it. then sort
        let i = 0;
        let found = false;
        while (!found) {
            // found it, then remove it from the array
            if (this.min[i] == temp) {
                found = true;
                this.min.splice(i, 1);
                return;
...
class slStack {
    ...
    // we used the arr which stores the values/data in sorted asc order
    getMin(stack) {
        if (stack.isEmpty()) { return null }
        else return this.mins[0];
    }
