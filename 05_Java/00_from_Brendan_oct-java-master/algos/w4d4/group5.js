...
class BST {
    ...
    delete(val, current) {
        ...
        if (find(val, current) == false) return;





        // we found it, current points to the node
        // with the value
        ...
        if (runner.val === val) {
            ...
            // we'll deal with this in a special way
            // remove the smallest in the right
            //  then swap it to the runner
            // or remove largest on the left and
            // swap with the runner
            removedNode = this.removeSmallest(runner.right);
            if (removedNode != null) {
                removedNode.right = this.root.right;
                removedNode.left = this.root.left;
                this.root = rmeovedNode;
            }
            else {
                removedNode = this.removeLargest(runner.left);
                if (removedNode != null) {
                    removedNode.right = this.root.right;
                    removedNode.left = this.root.left;
                    this.root = rmeovedNode;
    ...
    // if node found has no children
    if((current.left == null) && (current.right == null) {
    if (prev.left === val) prev.left = null;
}

if ((runner != null) && (runner.val === val){
    removedNode = this.removeSmallest(runner.right);
    if (removedNode != null) {
        removedNode.right = prev.right;
        removedNode.left = prev.left;
        prev = removedNode;
    }
    else {
        removedNode = this.removeLargest(runner.left);
