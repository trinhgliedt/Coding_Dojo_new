...
class DLList {
    ...
    exists(value) {
        ...
        var runner = this.head; // create a pointer to look through our list
        while (runner) { // while the runner is pointing towards a valid piece of data
            if (runner.data == value) { // check to see if the data of the current node matches the value we are looking for
                return true; // if they match return true and get out of the function
            } else { // otherwise
                runner = runner.next; // this line is for our forward movement so that we can continue searching
            }
        }
        return false; // if we do not find a match --> also if our runner is no longer
        // pointing at a valid node (end of the list) we didn't what we were looking for and return false
    }

    // reverse a doubly linked list
    // ideas:
    // we have a DLL and we want to reverse it --> what if we make a new variable that is a new instance of a DLL
    // once we have that new list --> go from the back to the front ('tail to head') of our populated list and push each
    // value into our new list
    // return the new list that will be the reverse of the original
    reverse() {
