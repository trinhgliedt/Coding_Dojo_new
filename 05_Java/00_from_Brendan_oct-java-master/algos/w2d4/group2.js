...
// using only one extra stack for storage, check if a given stack is sorted
// return the stack back to its original order when you are done
// assume node.data are integers
function isStackSorted(stack) {
    let isSorted = true;
...
function greaterOfTwoQueues(queue1, queue2) {
    let sum1 = 0;
    let sum2 = 0;
    let tQueue1 = new Queue();
    let tQueue2 = new Queue();


    while (!queue1.isEmpty()) {
        let node = queue1.dequeue();
        sum1 += node.data;
        tQueue1.enqueue(node);
    }
    while (!queue2.isEmpty()) {
        let node = queue2.dequeue();
        sum1 += node.data;
        tQueue2.enqueue(node);
    }
    while (!tQueue1.isEmpty()) {
        queue1.enqueue(tQueue1.dequeue());
    }
    while (!tQueue2.isEmpty()) {
        queue2.enqueue(tQueue2.dequeue());
    }
