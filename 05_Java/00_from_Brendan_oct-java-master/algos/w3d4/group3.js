...
function partitionDLL(DLL, pivot){
    ...
    // var newDLL = new DLL(); - create second DLL
    // run through list and remove all values greater than the pivot, adding them to the new DLL
    // finally, we return the values back into the original DLL
    if(DLL.size() == 0) {
        return;
    }
    var leftDLL = new DLL();
    var rightDLL = new DLL();
    for(var i = 0; i < DLL.size(); i++) {
        let temp = DLL.removeHead();
        if(temp.data > pivot) {
            rightDLL.addTail(temp);
        }
        else{
            leftDLL.addTail(temp);
        }
    }
    while(rightDLL != null){ // or for(var i = 0; i<rightDLL.size(); i++) {
        var temp = rightDLL.removeHead();
        leftDLL.addTail(temp);
    }
    return leftDLL;
