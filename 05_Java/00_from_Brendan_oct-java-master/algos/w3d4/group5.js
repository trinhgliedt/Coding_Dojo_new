...
function partitionDLL(DLL, pivot) {
    ...
    while (DLL) {
        if (fRunner.data < pivot) {
            var temp = removeVal(fRunner);
            addHead(temp);
            fRunner = fRunner.next
        }
        if (bRunner.data > pivot) {
            var temp2 = removeVal(bRunner);
            addTail(temp2);
        ...
        //use removeVal() from earlier this week. to pull.

        //use addHead() to put values to left of number

    }

    // var newDLL = new DLL();
    //go through the list pulling the values that are less than the pivot. put into newDLL1,
    //pull the values that are greater than the pivot value and put them into newDLL2.
    //put the values either in front of the pivot (if smaller than pivot), and to the right of the pivot if larger than pivot.
