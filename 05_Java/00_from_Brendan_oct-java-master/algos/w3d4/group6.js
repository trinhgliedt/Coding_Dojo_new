...
function partitionDLL(DLL, pivot){
    var lessThan = new DLList();
    var greaterThan = new DLList();
    var pivotNode = null;
    while (!DLL.isEmpty()) {
        var node = DLL.removeHead();
        if(node.data > pivot) {
            greaterThan.addTail(node);
        } else if (node.data == pivot){
            pivotNode = node;
        } else {
            lessThan.addTail(node);
        }
    }
    while (!lessThan.isEmpty()) {
        DLL.addTail(lessThan.removeHead());
    }
    DLL.addTail(pivotNode);
