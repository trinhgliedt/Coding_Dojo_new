...
function isPalindrome(queue) {
    ...
    // let frontData = this.front ? this.front.data : null;

    let dataArr1 = [];
    let newQueue = new Queue();
    // Creates an array copy
    while (!queue.isEmpty) {
        dataArr1.push(queue.checkFront());
        newQueue.enqueue(queue.dequeue());
    }

    // Puts the nodes back into original queue
    while (!newQueue.isEmpty) {
        queue.enqueue(newQueue.dequeue());
    }

    for (let i = 0; i < dataArr1.length / 2; i++) {
        if (!dataArr1[i] == dataArr1[dataArr1.length - 1 - i]) {
            return false;
        }
    }
    return true;
